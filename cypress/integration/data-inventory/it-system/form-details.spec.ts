...
context('RHEA>IT System>Details Form; fill fields', () => {
  ...
  const jsonData = '/data-inventory/it-system/form-details-data.json';

  let ui: ITSystem.ITSystemDetail;
  let api: ITSystemDetailAPI;
  let data: CreateRheaItSystems;

  //#region Delete IT System
    ...
    cy.get(ui.ownedBy.alias)
      ...
      .within($element => {
        CategoricalDropDown.selectItemOfCategory(
          systemOwner.search === '%' ? contextRecordPrefix : entityName,
  ...
  it('should save', () => {
    cy.server();
    cy.route('POST', '/api/hub/it-systems/*').as('postRequest');
    cy.get('[data-cy="save"').click();
    cy.wait('@postRequest');
    cy.get('li').contains('Data Inventory');
  });
  /*
  // should clone records
  it(`should clone record ${entityName}`, () => {
    cy.cloneEntityByName(entityName);
  });
  */
